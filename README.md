...
| `madrid_air_quality_Full_Dataset.ipynb` | `data/Madrid air quality full dataset/` (local Parquet files), with HuggingFace (`dmariaa70/METRAQ-Air-Quality`) as a fallback | Full ~64M row dataset read from the local Parquet folder when available, otherwise streamed from HuggingFace |

> **Note:** Both notebooks are identical in structure and code — only the data-loading cell differs. The Sample notebook's outputs were produced on the sample CSV; the Full Dataset notebook's outputs were produced on the full ~64M row Parquet dataset.
>
> **Note on the full dataset:** `madrid_air_quality_Full_Dataset.ipynb` first tries to read the local folder `data/Madrid air quality full dataset/` (10 Parquet files, ~544 MB total). If the folder is not present, it falls back to streaming from HuggingFace via `pd.read_parquet("hf://datasets/dmariaa70/METRAQ-Air-Quality")`. The HuggingFace fallback requires an internet connection and the `huggingface-hub` and `pyarrow` packages (both included in `requirements.txt`).
...
├── madrid_air_quality_Full_Dataset.ipynb     # Notebook — full dataset (local Parquet, HuggingFace fallback)
